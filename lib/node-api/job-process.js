...
exports.monitorNodes = async job => {
  ...
  for(const r of reports) {
    ...
    for(const monitor in t.monitors) {
      if(monitor.startsWith('ledgerNode')) {
        const {continuity: {
          localOpsListLength, latestSummary: {block: {blockHeight}}
        }} = t.monitors[monitor];
...
exports.startStage = async job => {
  const {data, queue} = job;
  const {dockerTag, id: stageId, ledgerNodeCount} = data;
  _logger.debug('starting stage', {stage: data});
  data.status = 'deployed';

  // data.vms = await _openstackApi.launchNetwork({ledgerNodeCount});
  data.vms = await _openstackApi.launchNetwork({dockerTag, ledgerNodeCount});
...
exports.stopStage = async job => {
  ...
  const destroyServersResult =
    await _openstackApi.destroyServers({instanceIds});
...
async function _sendElectorPool({targets}) {
  ...
  logger.debug('ElectorPool operatation.', {operation});
