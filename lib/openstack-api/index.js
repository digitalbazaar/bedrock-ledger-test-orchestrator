...
exports.launchNetwork = async ({dockerTag, ledgerNodeCount}) => {
  const auth = exports.getAuth();
  const ledgerNodes = [];

  // modify docker-compose
  const dockerCompose = require('./docker-compose');
  dockerCompose.service['bedrock-ledger-test'].image =
    `bedrock-ledger-test:${dockerTag}`;
  ...
  for(let i = 0; i < secondaryCount; ++i) {
    secondaries.push(
      limit(() => exports.launchSecondary(
        {auth, dockerComposeB64, primaryHostname})));
  }
...
exports.launchLedgerNode = async ({
  auth, dockerComposeB64, instanceConfig, name
}) => {
  ...
  const image = 'c58c3b27-43e5-42ac-b8e3-01886440d244';
  return testCloud.createServer({auth, cloudConfig, image, name});
};
...
exports.launchPrimary = async ({auth, dockerComposeB64}) => {
...
exports.launchSecondary = async ({auth, dockerComposeB64, primaryHostname}) => {
